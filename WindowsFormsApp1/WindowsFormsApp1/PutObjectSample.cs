...
namespace WindowsFormsApp1
{
    class PutObjectSample
    {
        ...
        public static string PutObjectFromFile(string fileToUpload, string bucketName,string bucketKey)
        {
            string msg = "";
         
            try
            {
                client.PutObject(bucketName, bucketKey, fileToUpload);
                msg = "[成功]Put object:{0} succeeded" + bucketKey;
            }
            catch (OssException ex)
            {
                msg = "[OSS异常]Failed with error code: {0}; Error info: {1}. \nRequestID:{2}\tHostID:{3}"+
                    ex.ErrorCode + ex.Message + ex.RequestId + ex.HostId;
            }
            catch (Exception ex)
            {
                msg = "[异常]Failed with error info: {0}" + ex.Message;
            }
            return msg;
        }
